...
class IMAudioVideoChat extends React.Component {
  ...
  handleIncomingCall = () => {
    if (this.state.isForeground && !this.callAccepted) {
      this.setState({
        isComInitiated: false,
      });
      InCallManager.startRingtone('_DEFAULT_');
  ...
  onEndCallAction = () => {
    // console.log(' is  be fired****', this.isForeground);
    // this.endCall(true)
  };
  ...
  onAnswerCallAction = (body) => {
    ...
    //RNCallKeep.endCall(body.callUUID);
  ...
  tryToRequestCommunication = () => {
    if (this.props.canReceiveMediaData) {
      ...
      const waitCanRecieveChannelDataChange = setInterval(() => {
        if (this.props.canReceiveMediaData && this.modalVisible) {
          this.requestCall();
          clearInterval(waitCanRecieveChannelDataChange);
        }
      }, 3000);
    }
  };
